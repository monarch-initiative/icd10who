...
https://urllib3.readthedocs.io/en/1.26.x/advanced-usage.htmx1l#ssl-warnings

# TODO: add: rdfs:seeAlso: and put the same URL as the namespace
...
# TODO: http://...purl/mondo/icd10who/... something like that
...
def save_rdf(d):
    ...
    # skos:prefLabel has been replaced by rdfs:label, but might still be in some cached items.
    ignore_keys = ['semantic_data', 'rdfs:subClassOf', 'rdfs:label', 'skos:prefLabel']
    ...
    graph.add((icd10who_node, RDFS.label, Literal('ICD10WHO')))
    graph.add((icd10who_node, RDFS.comment, Literal(about)))
    graph.add((icd10who_node, RDFS.seeAlso, Literal(str(ICD10WHO))))
    ...
    for k, v in kv_recursive_generator(d):
        ...
        # todo: eventually, this will just be rdfs:label, and can remove try/except
        try:
            label: str = v['semantic_data']['skos:prefLabel']
        except KeyError:
            label: str = v['semantic_data']['rdfs:label']
        ...
        graph.add((uri, RDFS.seeAlso, Literal(str(uri))))
